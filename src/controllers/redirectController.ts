      ...
      if (link.type === 'deeplink') {
        ...
        return res.send(`
          ...
          <head>
            <meta charset="UTF-8">
            <meta name="viewport" content="width=device-width, initial-scale=1.0">
            <title>Đang chuyển hướng tới ${deeplink.platform}...</title>
            <script src="https://cdn.tailwindcss.com"></script>
          </head>
          <body class="bg-[#0b101b] text-slate-100 flex flex-col items-center justify-center min-h-screen p-4 font-sans">
            <div class="max-w-md w-full bg-[#131b2e] border border-slate-700/60 rounded-3xl p-8 text-center shadow-2xl">
              ...
              <div class="relative w-20 h-20 mx-auto mb-6 flex items-center justify-center">
                ...
                <div class="w-12 h-12 rounded-full bg-gradient-to-tr from-[#06557c] to-[#32af5e] flex items-center justify-center text-white text-xl font-bold">
                  🚀
                </div>
              </div>

              <h2 class="text-xl font-bold mb-2">Đang mở ứng dụng ${deeplink.platform}...</h2>
              <p class="text-slate-400 text-xs mb-6">Hệ thống đang tự động kích hoạt ứng dụng trên điện thoại của bạn.</p>

              <div class="space-y-3">
                ${deeplink.appScheme ? `
                  <a href="${deeplink.appScheme}" class="block w-full py-3 px-4 bg-gradient-to-r from-[#06557c] to-[#32af5e] text-white font-medium rounded-2xl shadow-lg hover:opacity-95 transition active:scale-95">
                    Mở trong ứng dụng ${deeplink.platform}
                  </a>
                ` : ''}
                <a href="${destination}" class="block w-full py-3 px-4 bg-slate-800/80 hover:bg-slate-700/80 text-slate-300 font-medium rounded-2xl border border-slate-700 transition">
                  Tiếp tục qua trình duyệt web
                </a>
              </div>

              <div class="mt-8 pt-4 border-t border-slate-800 text-[11px] text-slate-500">
                Bảo vệ bởi hệ thống <span class="text-emerald-400 font-semibold">BoclinkVN</span>
              </div>
            </div>

            <script>
              const appScheme = ${JSON.stringify(deeplink.appScheme || null)};
              const webUrl = ${JSON.stringify(destination)};

              // Tự động kích hoạt schema app nếu có
              if (appScheme) {
                window.location.href = appScheme;
                // Fallback nếu sau 2.5s không mở được app thì chuyển sang web
                setTimeout(function() {
